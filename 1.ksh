...
do
  ...
  #extension check 
  if egrep -c '.*\.(vmdk|dayone|gnucash|pcap|bek|tpm|fve|asc|key|keypair|jks|pem|der|pfx|pk12|p12|pkcs12|mdf|sdf|sqldump|bak|wim|ova|ovf|cscfg|tfvars|dmp|pcap|cap|pcapng|cred|pass|kdbx|kdb|psafe3|kwallet|keychain|agilekeychain|cred|rdg|rtsz|rtsx|ovpn|tvopt|tblk|rdp|ppk|private_key|plist)' < "$line"; then
    : # do nothing
  ...
  #filepath check
  elif egrep -c '.*(irssi\\config|xchat2\\servlist_\\conf|aws\\credentials|\.purple\\accounts\.xml|\.gem\\credentials|config\\hub|REMINST\\SMSTemp|SMS\\data\\Variables.dat|SMS\\data\\Policy.xml|\.aws|doctl\\config.yaml|\.ssh\\|winscp).*' < "$line"; then    : # do nothing
    : # do nothing
