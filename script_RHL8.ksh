...
time (find "$scanpath" -type f -printf '%CY-%Cm-%Cd %CTu?%TY-%Tm-%Td %TTu?%AY-%Am-%Ad %ATu?%u?%M?%p\n' | tail -n +2 | sed 's/\"//g' | awk -v hostname="$hostname" -v hitpath="$hitpath" 'BEGIN { OFS="?" } { $1=gensub(/ /,"_","g",$1); $2=gensub(/ /,"_","g",$2); $3=gensub(/ /,"_","g",$3); $5=gensub(/ /,"_","g",$5); $6=gensub(/ /,"_","g",$6); $7=gensub(/ /,"_","g",$7); $8=gensub(/ /,"_","g",$8); $9=gensub(/ /,"_","g",$9); $10=gensub(/ /,"_","g",$10); $11=gensub(/ /,"_","g",$11); print $0, hostname }' | sed 's/"/_/g') >> "$logpath" 2>> "$errorpath"

find_name_end=$(date +%s)
